...
async def create_top5_graphic_international(stat_type, data, guild, bot):
    """Blue & white themed top 5 graphic for international leaderboard"""
    ...
    for i in range(-height, width + height, 60):
        stripe_draw.line([(i, 0), (i + height, height)], fill=(255, 255, 255, 12), width=20)
    ...
    # Title
    title_text = "TFH - ICC RUNS RANKINGS" if stat_type == "runs" else "TFH - ICC WKTS RANKINGS"
    title_label = "INTERNATIONAL CRICKET" 

    title_bbox = draw.textbbox((0, 0), title_text, font=title_font)
    title_width = title_bbox[2] - title_bbox[0]
    draw.text(((width - title_width) / 2, 30), title_text, fill=(255, 255, 255, 255), font=title_font)

    sub_bbox = draw.textbbox((0, 0), title_label, font=sub_font)
    sub_width = sub_bbox[2] - sub_bbox[0]
    # Draw pill behind subtitle
    pill_x = int((width - sub_width) / 2) - 20
    pill_y = 130
    pill_w = sub_width + 40
    pill_h = 45
    pill_img = Image.new('RGBA', (pill_w, pill_h), (0, 0, 0, 0))
    pill_draw = ImageDraw.Draw(pill_img)
    pill_draw.rounded_rectangle([(0, 0), (pill_w, pill_h)], radius=22, fill=(255, 255, 255, 50))
    img.paste(pill_img, (pill_x, pill_y), pill_img)
    draw = ImageDraw.Draw(img)
    draw.text(((width - sub_width) / 2, pill_y + 5), title_label, fill=(180, 220, 255, 255), font=sub_font)
    ...
    # Medal colors - blue/white international palette
    medal_colors = [
        (191, 153, 0),    # Bright cyan-blue (1st)
        (191, 115, 0),  # White (2nd)
        (94, 94, 93),  # Sky blue (3rd)
        (33, 82, 62),   # Medium blue (4th)
        (27, 50, 77),    # Deep blue (5th)
    ]
    medal_text_colors = [
        (255, 255, 255),
        (255, 255, 255),
    ...
    async with aiohttp.ClientSession() as session:
        for idx, row in enumerate(data[:5]):
            ...
            # Card with blue-white gradient
            card = Image.new('RGBA', (card_width, card_height), (0, 0, 0, 0))
            card_draw = ImageDraw.Draw(card)

            for y in range(card_height):
                ratio = y / card_height
                r = int(230 - (150 * ratio))  # 230 -> 80
                g = int(240 - (160 * ratio))  # 240 -> 80
                b = int(255 - (80 * ratio))   # 255 -> 175
                a = int(240 - (60 * ratio))
                card_draw.rectangle([(0, y), (card_width, y+1)], fill=(r, g, b, a))

            # Card border - bright cyan/white
            border_color = (0, 200, 255, 255) if idx == 0 else (255, 255, 255, 200)
            border_width = 5 if idx == 0 else 3
            ...
            accent_color = medal_colors[idx]
            accent = Image.new('RGBA', (card_width, 10), (0, 0, 0, 0))
            accent_draw = ImageDraw.Draw(accent)
            accent_draw.rounded_rectangle([(0, 0), (card_width, 10)], radius=5, fill=accent_color + (255,))
            ...
            rank_draw.text(
                ((rank_size - rank_tw) / 2, (rank_size - rank_th) / 2 - 7),
                rank_text, fill=medal_text_colors[idx] + (255,), font=stats_font
            )
            card.paste(rank_badge, (15, 15), rank_badge)
            ...
            if team_name:
                ...
                if team_name.lower() == "west indies":
                    try:
                        ...
                        circular_flag = Image.new('RGBA', (flag_size, flag_size), (0, 0, 0, 0))
                        circular_flag.paste(flag_img, (0, 0), mask)
                        card.paste(circular_flag, (card_width - flag_size - 15, 15), circular_flag)
                    except Exception as e:
                        print(f"Error loading West Indies flag: {e}")
                else:
                    flag_url = get_team_flag_url(team_name)
            ...
            # Player name - dark blue text on light card
            name_y = 530
            name_font_size = 52
            ...
            name_bbox = card_draw.textbbox((0, 0), player_name, font=fitted_name_font)
            name_width = name_bbox[2] - name_bbox[0]
            card_draw.text(((card_width - name_width) / 2, name_y), player_name, fill=(255, 255, 255), font=fitted_name_font)

            # Username - medium blue
            username_text = f"@{member.name}"
            username_y = 650
            username_bbox = card_draw.textbbox((0, 0), username_text, font=username_font)
            username_width = username_bbox[2] - username_bbox[0]
            ...
            # Stat - accent color
            stat_text = f"{row[1]} RUNS" if stat_type == "runs" else f"{row[1]} WICKETS"
            stat_y = 750
            stat_bbox = card_draw.textbbox((0, 0), stat_text, font=stats_font)
            stat_width = stat_bbox[2] - stat_bbox[0]
            card_draw.text(((card_width - stat_width) / 2, stat_y), stat_text, fill=(255, 255, 255), font=stats_font)
...
class CricketStats(commands.Cog):
    ...
    async def statsi_command(self, ctx, *, target: str = None):
        ...
        if target:
            ...
            if players:
                if len(players) > 1:
                    embed = discord.Embed(title="🔍 Multiple Players Found", color=0x1E90FF)
                    desc = f"Multiple players match '{target}':\n\n"
                    for i, (player, team) in enumerate(zip(players, team_names), 1):
                        flag = get_team_flag(team)
                        desc += f"**{i}.** {flag} **{player['name']}** - {team}\n"
